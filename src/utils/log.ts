/**
 * Code from windicss/windicss-intellisense
 *
 * @see https://github.com/windicss/windicss-intellisense/blob/main/src/utils/console.ts
 */
...
export class Log {
  ...
  static get outputChannel(): OutputChannel {
    if (!this._channel) {
      this._channel = window.createOutputChannel(EXTENSION_NAME);
    }
  ...
  static async error(
    ...
    if (prompt) {
      ...
      const message =
        typeof err === "string"
          ? err
          : `${EXTENSION_NAME} Error: ${err.toString()}`;
